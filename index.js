...
function buildRoomButtons(
  ...
  const mk = (id, label, emoji, style) => {
    const button = new ButtonBuilder().setCustomId(id).setEmoji(emoji).setStyle(style);
  ...
  return [
    ...
    new ActionRowBuilder().addComponents(
      ...
      mk('room_clear_chat', '', '🧹', ButtonStyle.Secondary)
    ),
    new ActionRowBuilder().addComponents(
...
function isProtectedRoomControlMessage(message) {
  return (
    isRoomPanelMessage(message) ||
    isRoomAuxMessage(message) ||
    isRoomTrustedContinuationMessage(message)
  );
}
